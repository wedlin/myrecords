...
```bash
quick debug¡Gecho
formal debug¡Gbbwarn / bbplain
parameter tracking¡Gbitbake -e
e.g.
...
bb.debug(1, "message") ¡÷ debug level¡Anumber bigger more clear

e.g.
bb.plain("=== Debug: Srouce path = %s" % d.getVar("S"))
bb.warn("Warnning¡Gthe Build path = %s" % d.getVar("B"))
...
```bash
BitBake every task(task¡Ae.g. do_fetch, do_unpack, do_patch, do_configure etc...¡^
every taks have stamp file¡AUsed to record whether this task has been successfully executed¡G
The path is usually:
