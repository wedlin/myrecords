Ôª? OpenBMC Development Structure Demo

## 1Ô∏è‚É£ Directory Structure
```
igsobmc/                   ??Main repository
?ú‚??Ä openbmc_ast2700/       ??OpenBMC + AST2700 patch
?ú‚??Ä openbmc-upstream/      ??Upstream OpenBMC
?ú‚??Ä igs_ob_mid/            ??Middleware layer
??  ?ú‚??Ä meta-igs-ast2700/  ??AST2700 BSP extension layer (e.g., AST2700 patch)
??  ?î‚??Ä meta-igs-common/   ??Common layer (e.g., libgcrypt patch)
?î‚??Ä igs_ob_prj/            ??Project-specific layer
    ?ú‚??Ä meta-igs-prjA/     ??Project A layer (emulate to use AST2700 patch of middleware)
    ?ú‚??Ä meta-igs-prjB/     ??Project B layer (emulate to use OpenBMC build-in AST2700 support)
    ?î‚??Ä setup/             ??Build setup scripts
```

---

## 2Ô∏è‚É£ Git Repository Creation
...
## 3Ô∏è‚É£ Summary of Git Operations
| Layer/Repo                     | Action                          |
|--------------------------------|--------------------------------|
...
---
## 5Ô∏è‚É£ How to Update a Submodule and Remove Temporary Build Directories
...
```
> `--cached` removes the directories from Git?ôs index only (it won?ôt delete local files if they still exist).
